...
def home(request):
    if 'user_id'  in request.session:
        ...
        latest_5projects = Projects.objects.all().order_by('-id')[:5]
        context['latest_5projects'] = latest_5projects
        Feature=Featured.objects.all()
        arr=[]
        for f in Feature:
             F = Projects.objects.get(id=f.pro_id)
             arr.append(F)
...
def pro(request):
    if 'user_id'  in request.session:
        ...
        context['facebook'] = user.facebook_profile
...
def update(request):
    if 'user_id'  in request.session:
        ...
        user.facebook_profile = request.POST['facebook']
        user.country=request.POST['country']
        user.birthdate= request.POST['birthdate']
        if len(request.FILES)!=0:
            if len(user.profile_img):
                os.remove(user.profile_img.path)


            user.profile_img=request.FILES['pimage']
